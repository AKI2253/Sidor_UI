...
// 信封格式与官方 WebConnectionRpc 一致（0.1.5 起）：
//   POST /api/<a>/<b>，body = { type:'client-request', rpcId, method:'<a>/<b>', payload:{ args } }
//   应答 = { type:'server-response', rpcId, result:{ ok, value | error } }
// endpoint 用斜杠形式（如 'session/list'）；args 是该远端方法的命名参数对象，
// 字段名必须与官方方法的形参名逐一对应（网关按 descriptor 校验）：
// 无参方法传 {}（directoryPicker/pick）、形状参名如 session/list 的 { _request }、
// session/prompt 的 { request }。
// 0.1.4 及更早使用点号方法名 + 裸 payload，二者在 0.1.5 网关下均被拒绝。
async function sidorHostRpc(endpoint, args) {
  const rpcId = sidorUuid()
  ...
  if (!res.ok) throw new Error('host ' + endpoint + ' HTTP ' + res.status)
  const json = await res.json()
  if (!json || json.type !== 'server-response' || json.rpcId !== rpcId) {
    throw new Error('host ' + endpoint + ' 响应无效')
  }
...
// 0.1.5 起 workspace 清单不再是 unary RPC（workspace/list 已移除，改为流式
// workspace/follow），而会话摘要 session/list 自带 cwd，直接取用。
async function sidorSessionWorkspacePath(sessionId) {
  if (!sessionId) return null
  try {
    const s = await sidorHostRpc('session/list', { _request: {} })
  ...
  apply(ctx) {
    ...
    function FilePickButton(props) {
      ...
      const pickFolder = async () => {
        ...
        setPicking(true)
        try {
          // 0.1.5 起目录选择改为 directoryPicker 命名空间的远端方法，
          // pick 无参数并直接返回绝对路径字符串（取消为 null）。
          const value = await sidorHostRpc('directoryPicker/pick', {})
          const p = typeof value === 'string' && value !== '' ? value : null
          if (p) appendPaths(p)
          else show('已取消选择')
        } catch (err) {
