 ...
 *   在静态形态下的处理：
 *     - balance-* 由 Client 半以降级方式实现（localStorage + 浏览器 fetch 直连）；
 *     - upload-doc（文件落盘）静态形态**不可用**：「文档」按钮降级为路径选择，
 *       经同源 /api RPC 直连官方 host.pickDirectory / workspace.list 等，把路径
 *       贴入输入框由 agent 工具读取（见 src/sidor-fx-client.js 顶部注释）。
