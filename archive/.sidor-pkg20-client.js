return {
  ...
  apply(ctx) {
    ...
    function MainUI(props) {
      ...
      const composerEnter = () => {
        const sid = ssState && ssState.current ? ssState.current : (wsSessions[0] && wsSessions[0].id)
        if (sid) { openSession(sid) } else { newSession() }
      }
      ...
      return React.createElement('div', { className: 'sid-main-ui' },
        ...
        React.createElement('div', { className: 'sid-body' },
          ...
          React.createElement('div', { className: 'sid-stage' },
            ...
            React.createElement('div', { className: 'sid-composer' },
              React.createElement('input', {
                type: 'text',
                placeholder: T.composer,
                onKeyDown: (e) => { if (e.key === 'Enter') { e.preventDefault(); composerEnter() } },
              }),
            ),
          ),
        ),
...
.sid-composer { width: min(560px, 100%); }
.sid-composer input {
  width: 100%; height: 44px; padding: 0 16px; border: 1px solid var(--sid-hairline-strong); border-radius: 12px;
  background: color-mix(in srgb, var(--sid-surface) 72%, transparent); color: var(--sid-ink);
  font-size: 14px; outline: none;
  backdrop-filter: blur(8px); -webkit-backdrop-filter: blur(8px);
}
.sid-composer input:focus { box-shadow: 0 0 0 1px var(--sid-ink); }
.sid-composer input::placeholder { color: var(--sid-ink-dim); opacity: 0.7; }
.sid-strip {
